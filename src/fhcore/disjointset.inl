...
inline DisjointSetT<Test, size>::DisjointSetT(IBoard * pBoard)
    : pBoard(pBoard) {
}
...
template<typename Test, coord_t size>
inline IDisjointSet * DisjointSetT<Test, size>::create(IBoard *pBoard) {
    return new DisjointSetT(pBoard);
}

template<typename Test, coord_t size>
inline Color DisjointSetT<Test, size>::get(int index) {
    return status[index];
}
...
template<typename Test, coord_t size>
inline Color & DisjointSetT<Test, size>::operator[](int index) {
    return status[index];
}
