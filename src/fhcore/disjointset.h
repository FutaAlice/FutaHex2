...
public:
    virtual color::Color get(int index) = 0;
    virtual void set(int index) = 0;
    virtual color::Color & operator[](int index) = 0;
    virtual void revert() = 0;
    virtual void ufinit() = 0;
    virtual bool check_after_set(int center_index, const Color color) = 0;
public:
    color::Color color_to_move() { return static_cast<Color>(rounds & 1); }
protected:
    size_t rounds;
};
...
public:
    static IDisjointSet * create(board::IBoard *pBoard);
public:
    virtual color::Color get(int index);
    virtual void set(int index);
    virtual color::Color & operator[](int index);
    virtual void revert();
    virtual void ufinit();
